# Catcat — Roadmap (พอร์ตจาก workflow.md มาที่ Rust/Axum)

> ที่มา: `workflow.md` เป็นแผนของโปรเจกต์ **BoxPhone (Node.js)** ทุก `server.js:NNN` / `app.js:NNN`
> ในไฟล์นั้น **ไม่มีอยู่จริงใน catcat** ไฟล์นี้คือแผนเดียวกันแต่แม็ปกับโค้ด Rust จริง
...
## A. Gap Analysis — catcat วันนี้ vs แผน

catcat **ดิบกว่า** BoxPhone โดยรวม แต่ **ล้ำกว่า 1 จุด** (lazy start ฝั่ง server)

| ความต้องการ (จาก workflow.md) | catcat วันนี้ | สถานะ | จุดในโค้ด |
|---|---|---|---|
...
## B. สถาปัตยกรรมเป้าหมาย (ยกมาจาก workflow.md §B — ยังถูกต้องสำหรับ catcat)
...
catcat ได้เปรียบ: ฝั่ง server lazy อยู่แล้ว เหลือแค่ทำให้ **client** ไม่เปิด decoder ใน grid

---

## C. Backlog ของ catcat (เรียงใหม่ — foundation-first)

### P0 — ฐาน (ปลดบล็อกก่อนทำอย่างอื่น)
1. **Verify ฐาน 1 เครื่อง** — รัน catcat จริง, ยืนยัน video decode + tap ทำงานในเบราว์เซอร์
...
> ของที่ workflow.md ขึ้น ✅ (admin page, hot-plug, MCP server, layout preset) **ยังไม่มีใน catcat**
